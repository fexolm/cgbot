        ...
        impl<'a> Simulation<'a> {
            ...
            fn adjust_drone_move(
                ...
                for rotation in [(-PI / 3.), (PI / 3.), (2. * PI / 3.), (-2. * PI / 3.)] {
                    ...
                    let mov = dir * 600.;
                    let new_pos =
                        (drone.pos + mov).clamp(Vec2::new(0., 0.), Vec2::new(9999., 9999.));
                    drone.dead = self
                        .tracker
                        .monsters
                        .values()
                ...
                for i in 0..2 {
                    ...
                    if drone.pos.y < 400. {
                        ...
                        state.score.saving_urgent_scans_score +=
                            (drone.urgent_scans_cost * drone.urgent_scans_cost) as f32
                                / iter
                                / iter;
            ...
            fn simulate_all(&mut self, state: &mut GameState, gene: &mut Gene) {
                ...
                state.score.ascent_score += state.drones[0].base_scans_cost as f32
                    * (1. - state.drones[0].pos.y / 10000.)
                    / 5.;
                state.score.ascent_score += state.drones[1].base_scans_cost as f32
                    * (1. - state.drones[1].pos.y / 10000.)
                    / 5.;
    ...
    pub mod strategy {
        ...
        impl Strategy {
            pub fn play(&mut self, world: &World) {
                ...
                let actions = self.pathfinding.search(
                    world,
                    &self.tracker,
